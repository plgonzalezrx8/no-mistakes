...
The installer drops the binary in `~/.no-mistakes/bin` and links it into `~/.local/bin` or `/usr/local/bin`. The daemon is started later by `no-mistakes init`, `no-mistakes attach`, `no-mistakes rerun`, `no-mistakes update`, or an explicit `no-mistakes daemon start`.
